...
| 类型 | 形状 | 颜色 | 大小 | 说明 |
|------|------|------|------|------|
| `project` | 星形 (star) | 金色 `#f59e0b` | 40 | 项目根节点，居中 |
| `module` | 菱形 (diamond) | 绿色 `#059669` | 25 | 功能模块 |
| `main-task` | 圆形 (dot) | 按状态着色 | 22 | 主任务/开发阶段 |
| `sub-task` | 小圆点 (dot) | 按状态着色 | 10 | 子任务/具体工作项 |
| `document` | 方形 (box) | 紫色 `#7c3aed` | 16 | 文档片段 |
...
页面每 **15 秒**静默刷新数据（不重新创建图谱布局，仅更新节点颜色和属性），确保 MCP 工具修改的任务状态能实时反映到图谱上。
