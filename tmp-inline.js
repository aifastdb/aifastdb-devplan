...
function loadVisNetwork(index) {
  if (index >= VIS_URLS.length) {
    ...
    document.getElementById('loading').innerHTML = '<div style="text-align:center"><div style="font-size:48px;margin-bottom:16px;">??</div><p style="color:#f87171;">渲染引擎加载失败</p><p style="color:#9ca3af;margin-top:8px;font-size:13px;">所有 CDN 均不可用</p><button class="refresh-btn" onclick="location.reload()" style="margin-top:12px;">刷新页面</button></div>';
    return;
  }
...
function renderGraph() {
  try {
    ...
    document.getElementById('loading').innerHTML = '<div style="text-align:center"><div style="font-size:48px;margin-bottom:16px;">??</div><p style="color:#f87171;">渲染失败: ' + err.message + '</p><button class="refresh-btn" onclick="loadData()" style="margin-top:12px;">重试</button></div>';
  }
}
...
function loadDataFull() {
  ...
  Promise.all([
    ...
    document.getElementById('loading').innerHTML = '<div style="text-align:center"><div style="font-size:48px;margin-bottom:16px;">??</div><p style="color:#f87171;">数据加载失败: ' + err.message + '</p><button class="refresh-btn" onclick="loadData()" style="margin-top:12px;">重试</button></div>';
  });
}
...
function renderDocContent(doc, section, subSection) {
  ...
  if (doc.parentDoc) {
    ...
    contentHtml += '<div class="docs-related-title">?? 父文档</div>';
  ...
  if (childDocs.length > 0) {
    contentHtml += '<div class="docs-related" style="margin-top: 12px;">';
    contentHtml += '<div class="docs-related-title">?? 子文档 (' + childDocs.length + ')</div>';
...
function startBatchImport(mdFiles) {
  ...
  function finish() {
    ...
    summaryEl.innerHTML = '<div style="font-weight:600;margin-bottom:6px;">' + (cancelled ? '?? 导入中断' : '? 导入完成') + '</div>'
...
function loadCodeIntelPage(forceRefresh) {
  ...
  Promise.all([
    ...
    cardsEl.innerHTML = statCard('??', '0', 'Code Intelligence', '加载失败', 'rose');
  ...
  if (recommended) {
    ...
    if (!communities.length) {
      ...
      for (var c = 0; c < communities.length; c++) {
        ...
        html += '<button class="refresh-btn" data-community-id="' + escHtml(encodeURIComponent(String((item.community && item.community.id) || ''))) + '" onclick="applyCodeIntelModuleRecommendation(decodeURIComponent(this.getAttribute('data-community-id') || ''))" style="padding:6px 10px;">应用</button>';
        html += '</div>';
...
function renderCodeIntelGraph(graph) {
  ...
  if (USE_3D && typeof ForceGraph3D !== 'undefined') {
    renderCodeIntelGraph3D(renderGraph, container);
    return;
  }
  ...
  for (var i = 0; i < renderGraph.nodes.length; i++) {
    ...
    var bg = '#64748b';
    var shape = 'dot';
    if (n.type === 'file') { bg = '#3b82f6'; shape = 'box'; }
    else if (n.type === 'folder') { bg = '#64748b'; shape = 'diamond'; }
    else if (n.type === 'symbol') { bg = '#22c55e'; shape = 'dot'; }
    else if (n.type === 'community') { bg = '#f59e0b'; shape = 'hexagon'; }
    else if (n.type === 'process') { bg = '#a855f7'; shape = 'star'; }
    else if (n.type === 'module') { bg = '#fb923c'; shape = 'triangle'; }
    else if (n.type === 'project') { bg = '#ef4444'; shape = 'ellipse'; }
    nodes.push({
      id: n.id,
      label: n.label,
      ...
      size: n.type === 'community' ? 22 : (n.type === 'process' ? 20 : 14),
    });
  }
  ...
  for (var j = 0; j < renderGraph.edges.length; j++) {
    ...
    var color = '#475569';
    var bridgeKind = String((e.properties && e.properties.bridgeKind) || '');
    if (e.label === 'IMPORTS') color = '#60a5fa';
    else if (e.label === 'DEFINES') color = '#34d399';
    else if (e.label === 'MEMBER_OF') color = '#f59e0b';
    else if (e.label === 'STEP_IN_PROCESS') color = '#c084fc';
    else if (e.label === 'BRIDGE_LINK') color = '#fb923c';
    else if (e.label === 'BRIDGE_RECOMMEND') color = (bridgeKind === 'recommended-process' ? '#38bdf8' : '#facc15');
    edges.push({
      from: e.from,
      to: e.to,
      arrows: 'to',
      width: (e.label === 'BRIDGE_LINK' ? 2.6 : (bridgeKind === 'recommended-process' ? 2.2 : (e.label === 'BRIDGE_RECOMMEND' ? 1.8 : 1.2))),
      dashes: (e.label === 'BRIDGE_RECOMMEND'),
      color: { color: color, opacity: (e.label === 'BRIDGE_LINK' ? 0.9 : 0.65) },
      title: bridgeKind || e.label
    });
  }
...
function prepareCodeIntelGraphView(graph) {
  if (USE_3D || !graph || !graph.nodes || graph.nodes.length <= CODE_INTEL_2D_NODE_LIMIT) {
    return {
      graph: graph,
      metaText: (graph.nodes.length + ' 节点 / ' + graph.edges.length + ' 边')
    };
  }
  ...
  for (var j = 0; j < graph.edges.length; j++) {
    var e = graph.edges[j];
    if (e.label === 'STEP_IN_PROCESS') continue;
  ...
  return {
    ...
    metaText: graph.nodes.length + ' 节点 / ' + graph.edges.length + ' 边 (2D 概览模式: ' + keptNodes.length + ' / ' + keptEdges.length + ')'
  };
}
...
function renderCodeIntelClusters(clusters) {
  ...
  for (var i = 0; i < clusters.length; i++) {
    var c = clusters[i];
    html += '<div style="padding:10px 0;border-bottom:1px solid #1f2a44;">';
    html += '<div style="font-weight:700;color:#dbe7ff;">' + escHtml(c.label || c.id) + '</div>';
    html += '<div style="margin-top:4px;color:#8da2d6;">文件 ' + (c.fileCount || 0) + ' / 符号 ' + (c.symbolCount || 0) + '</div>';
    if (c.members && c.members.length) {
      html += '<div style="margin-top:6px;font-size:11px;color:#94a3b8;">' + escHtml(c.members.slice(0, 4).join(', ')) + (c.members.length > 4 ? ' ...' : '') + '</div>';
    }
    html += '</div>';
  }
  el.innerHTML = html;
...
function renderCodeIntelProcesses(processes) {
  ...
  for (var i = 0; i < processes.length; i++) {
    var p = processes[i];
    html += '<div style="padding:10px 0;border-bottom:1px solid #1f2a44;">';
    html += '<div style="font-weight:700;color:#dbe7ff;">' + escHtml(p.label || p.id) + '</div>';
    html += '<div style="margin-top:4px;color:#8da2d6;">stepCount: ' + (p.stepCount || 0) + '</div>';
    if (p.steps && p.steps.length) {
      html += '<div style="margin-top:6px;font-size:11px;color:#94a3b8;">' + escHtml(p.steps.slice(0, 4).join(' -> ')) + (p.steps.length > 4 ? ' ...' : '') + '</div>';
    }
    html += '</div>';
  }
  el.innerHTML = html;
...
function renderStatsPage(data) {
  ...
  else if (pct > 0) motivate = '??? 万事开头难，已迈出第一步！';
...
function startAiBatchGenerate() {
  ...
  fetch('/api/batch/config').then(function(r) { return r.json(); }).then(function(cfg) {
    ...
    if (statusEl) statusEl.textContent = '?? 加载配置失败，使用默认值';
...
function runBatchIntegrityCheck(preparedResults, summaryEl) {
  ...
  fetch('/api/batch/verify', {
    ...
    if (statusEl) {
      if (s.errors > 0) {
        statusEl.textContent = '?? 完整性检测发现 ' + s.errors + ' 个错误';
    ...
    if (s.warnings > 0) html += '<span style="color:#f59e0b;">?? 警告: ' + s.warnings + '</span>';
      ...
      for (var qi = 0; qi < problemResults.length; qi++) {
        ...
        if (r.warnings && r.warnings.length > 0) {
          for (var wi = 0; wi < r.warnings.length; wi++) {
            html += '<div class="batch-verify-issue warning">?? ' + r.warnings[wi] + '</div>';
          }
        }
...
function checkSingleMemoryIntegrity(memoryId) {
  ...
  fetch('/api/batch/verify', {
    ...
    var statusIcon = r.status === 'error' ? '?' : (r.status === 'warning' ? '??' : '?');
    ...
    if (r.warnings && r.warnings.length > 0) {
      for (var i = 0; i < r.warnings.length; i++) {
        html += '<div class="verify-issue">?? ' + r.warnings[i] + '</div>';
      }
    }
...
function checkAllMemoriesIntegrity() {
  ...
  fetch('/api/batch/verify', {
    ...
    if (s.warnings > 0 || s.errors > 0) {
      ...
      if (eb.memoryType > 0) html += '<span style="color:#ef4444;">??? 非法 Type: <strong>' + eb.memoryType + '</strong> <span style="color:#6b7280;">(可修复)</span></span>';
      ...
      mdvLoadScript(MDV_MERMAID_URLS, 0, function(mermaidOk) {
        ...
        if (statusEl) {
          if (markedOk) {
            ...
            statusEl.textContent = '?? CDN 加载失败，使用简易渲染';
    ...
    (function(codeEl) {
      ...
      try {
        mermaid.render(containerId, source.trim()).then(function(result) {
          ...
          errDiv.textContent = '?? Mermaid 渲染失败: ' + (err.message || err);
