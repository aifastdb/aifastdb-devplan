...
// Initialize settings page — use runtime RENDERER_ENGINE (reflects fallback)
(function() { syncEngineUI(RENDERER_ENGINE); })();
...
function isAssetReady(checkFn) {
  if (typeof checkFn !== 'function') return true;
  try { return !!checkFn(); } catch (e) { return false; }
}

function getAssetLoadCandidates(localUrl, cdnUrls) {
  var candidates = [];
  var cdnList = Array.isArray(cdnUrls) ? cdnUrls : [];
  if (ASSET_LOAD_MODE === 'cdn-first') {
    for (var i = 0; i < cdnList.length; i++) {
      if (cdnList[i]) candidates.push({ url: cdnList[i], source: 'cdn' });
    }
    if (localUrl) candidates.push({ url: localUrl, source: 'local' });
    return candidates;
  }
  if (localUrl) candidates.push({ url: localUrl, source: 'local' });
  for (var j = 0; j < cdnList.length; j++) {
    if (cdnList[j]) candidates.push({ url: cdnList[j], source: 'cdn' });
  }
  return candidates;
}

function loadScriptWithFallbacks(options, callback) {
  options = options || {};
  var label = options.label || 'script';
  var timeoutMs = options.timeoutMs || ASSET_LOAD_TIMEOUT_MS;
  var candidates = getAssetLoadCandidates(options.localUrl, options.cdnUrls);
  ...
  function tryLoad(index) {
    ...
    function tryNext(reason) {
      ...
      log(label + ' [' + candidate.source + '] ' + reason + '，尝试下一个来源', false);
      tryLoad(index + 1);
    }
...
// Sync engine badge + settings radio cards to actual value
function syncEngineUI(value) {
  var label = document.getElementById('engineNameLabel');
  if (label) label.textContent = value === '3d' ? '3D Force Graph' : 'vis-network';
...
// ── 3D 引擎资源（本地优先，失败后回退 CDN）──
var LOCAL_THREE_JS_URL = '/vendor/three.min.js';
var LOCAL_FORCE_GRAPH_URL = '/vendor/3d-force-graph.min.js';
var THREE_JS_CDN_URLS = [
  'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js',
  'https://unpkg.com/three@0.160.0/build/three.min.js'
];
var FORCE_GRAPH_CDN_URLS = [
  'https://cdn.jsdelivr.net/npm/3d-force-graph@1.78.4/dist/3d-force-graph.min.js',
  'https://unpkg.com/3d-force-graph@1.78.4/dist/3d-force-graph.min.js'
];
var VIS_NETWORK_CDN_URLS = [
...
function loadThreeJS() {
  loadScriptWithFallbacks({
    label: 'Three.js',
    localUrl: LOCAL_THREE_JS_URL,
    cdnUrls: THREE_JS_CDN_URLS,
...
function loadForceGraph3D() {
  loadScriptWithFallbacks({
    ...
    localUrl: LOCAL_FORCE_GRAPH_URL,
    cdnUrls: FORCE_GRAPH_CDN_URLS,
...
// ── vis-network 资源（本地优先，失败后回退 CDN）──
var LOCAL_VIS_NETWORK_URL = '/vendor/vis-network.min.js';
