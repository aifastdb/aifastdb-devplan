...
export function getStyles(): string {
  return `
     ...
     * Reference: awesome-design-md/design-md/linear.app/DESIGN.md
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { background: var(--ds-app-bg); color: #e5e7eb; font-family: -apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', Roboto, sans-serif; overflow: hidden; transition: background-color 0.18s ease; }
     ...
     * canvas #010102 · hairline #23252a · lavender accent #5e6ad2
     * surface-lift hover (no full-bg swap) · pill-shaped active state
     * ============================================================ */
    .sidebar {
      ...
      transition: width 0.24s cubic-bezier(0.32, 0.72, 0.24, 1), background-color 0.18s ease, border-color 0.18s ease;
      overflow: hidden;
      z-index: 40;
    }
    .sidebar.expanded { width: var(--ds-sidebar-w-expanded); }

    /* Header — single accent dot + product name, no rainbow gradient */
    .sidebar-header {
      ...
      overflow: hidden;
      padding: 0 16px;
      transition: background 0.16s ease, border-color 0.18s ease;
    }
    .sidebar:not(.expanded) .sidebar-header { padding: 0; align-items: center; justify-content: center; }
    .sidebar-header:hover { background: var(--ds-surface-1); }
    ...
    .sidebar-menu-icon svg { width: 18px; height: 18px; display: block; }
    .sidebar:not(.expanded) .sidebar-menu-icon {
      width: var(--ds-sidebar-item-h);
      height: var(--ds-sidebar-item-h);
      margin: 0 auto;
    }
    .sidebar-header:hover .sidebar-menu-icon { color: var(--ds-ink); background: var(--ds-surface-2); }
    ...
    .sidebar-logo-short { display: inline-flex; align-items: center; justify-content: center; }
    .sidebar.expanded .sidebar-logo-full { display: inline-flex; }
    ...
    .sidebar-nav::-webkit-scrollbar-thumb { background: var(--ds-hairline-strong); border-radius: 2px; }
    .sidebar-footer {
      padding: var(--ds-space-xs);
      transition: border-color 0.18s ease;
    }

    /* Nav item — pill on surface-1 lift, lavender ink + dot for active */
    .nav-item {
      position: relative;
      display: flex;
      align-items: center;
    ...
    .nav-item.active {
      ...
      box-shadow: inset 0 0 0 1px var(--ds-hairline-strong);
    }
    .nav-item.active .nav-item-icon { color: var(--ds-primary); }
    .nav-item.disabled { cursor: default; opacity: 0.4; }
    .nav-item.disabled:hover { background: transparent; color: var(--ds-ink-subtle); }
    ...
    .nav-item-icon svg { width: 16px; height: 16px; display: block; stroke: currentColor; stroke-width: 1.6; }
    ...
    /* Tooltip (collapsed mode) — surface-2 panel with hairline */
    ...
    /* Graph — flex 自适应高度 */
    .graph-container { position: relative; flex: 1; background: var(--ds-app-bg); min-height: 0; transition: background-color 0.18s ease; }
    ...
    /* Detail Panel */
    .panel { position: absolute; top: 12px; right: 12px; width: 340px; max-height: calc(100vh - 180px); background: var(--ds-app-bg-elevated); border: 1px solid var(--ds-app-border); border-radius: 12px; box-shadow: 0 20px 40px rgba(0,0,0,0.5); z-index: 10; display: none; overflow: hidden; min-width: 280px; max-width: calc(100vw - 40px); transition: background-color 0.18s ease, border-color 0.18s ease; }
    ...
    /* Legend */
    .legend { background: var(--ds-app-bg-elevated); border-top: 1px solid var(--ds-app-border); padding: 6px 24px; display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 10px 20px; font-size: 12px; color: #9ca3af; transition: background-color 0.18s ease, border-color 0.18s ease; }
    ...
    .page-code-intel { position: relative; background: var(--ds-app-bg); }
    ...
    /* Settings Page */
    .settings-page { padding: 32px 48px; overflow-y: auto; background: var(--ds-app-bg); flex: 1; width: 100%; display: flex; flex-direction: column; align-items: center; transition: background-color 0.18s ease; }
    ...
    /* App-theme radio cards — extra swatch row */
    .app-theme-group { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    @media (max-width: 720px) { .app-theme-group { grid-template-columns: 1fr; } }
    .app-theme-card .radio-label { flex-wrap: wrap; gap: 8px; }
    .theme-swatch-row { display: inline-flex; align-items: center; gap: 4px; margin-left: auto; padding: 2px 6px; border-radius: var(--ds-radius-pill); background: rgba(255,255,255,0.04); }
    .theme-swatch { width: 12px; height: 12px; border-radius: 50%; border: 1px solid transparent; display: inline-block; }
    .theme-swatch-accent { box-shadow: 0 0 6px currentColor; }
    ...
    .ga-card { border: 1px solid #374151; border-radius: 10px; padding: 12px; background: #0b1220; }
    .ga-card.ok { border-color: rgba(16,185,129,0.45); background: rgba(16,185,129,0.08); }
    .ga-card.warn { border-color: rgba(245,158,11,0.45); background: rgba(245,158,11,0.08); }
    .ga-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 8px; }
    .ga-title { font-size: 13px; color: #e5e7eb; font-weight: 600; }
    .ga-lamp { width: 10px; height: 10px; border-radius: 50%; display: inline-block; margin-right: 6px; }
    .ga-lamp.green { background: #10b981; box-shadow: 0 0 8px rgba(16,185,129,0.7); }
    .ga-lamp.yellow { background: #f59e0b; box-shadow: 0 0 8px rgba(245,158,11,0.7); }
    .ga-meta { font-size: 11px; color: #9ca3af; line-height: 1.6; }
    .ga-reason { margin-top: 6px; font-size: 11px; color: #fbbf24; }
    .ga-refresh { margin-top: 10px; font-size: 11px; padding: 4px 10px; border: 1px solid #374151; border-radius: 6px; background: transparent; color: #9ca3af; cursor: pointer; }
    .ga-refresh:hover { background: #1e293b; color: #e5e7eb; }

    /* ===== Memory Browser ===== */
    .memory-page { padding: 24px 32px; overflow-y: auto; background: var(--ds-app-bg); flex: 1; display: flex; flex-direction: column; transition: background-color 0.18s ease; }
    ...
    /* Phase-70: 生成记忆按钮（单按钮，直接触发 AI 批量生成） */
    .memory-generate-btn { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: #fff; border: none; border-radius: 8px; padding: 7px 16px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.2s; white-space: nowrap; margin-right: 10px; }
    ...
    /* Stats Dashboard */
    .stats-page { padding: 24px; overflow-y: auto; background: var(--ds-app-bg); flex: 1; transition: background-color 0.18s ease; }
    ...
    /* ===== Docs Browser Page ===== */
    .docs-page { display: flex; flex: 1; min-height: 0; overflow: hidden; background: var(--ds-app-bg); transition: background-color 0.18s ease; }
    .docs-sidebar { position: relative; width: 280px; background: var(--ds-app-bg-elevated); border-right: 1px solid var(--ds-app-border); display: flex; flex-direction: column; flex-shrink: 0; overflow: hidden; min-width: 240px; transition: background-color 0.18s ease, border-color 0.18s ease; }
    .docs-sidebar-header { padding: 16px 20px 12px; border-bottom: 1px solid var(--ds-app-border); flex-shrink: 0; transition: border-color 0.18s ease; }
    ...
    .docs-add-btn { background: rgba(99,102,241,0.15); border: 1px solid rgba(99,102,241,0.3); color: #a5b4fc; font-size: 11px; padding: 3px 10px; border-radius: 5px; cursor: pointer; transition: all 0.15s; font-weight: 500; white-space: nowrap; }
    .docs-add-btn:hover { background: rgba(99,102,241,0.3); color: #c7d2fe; border-color: rgba(99,102,241,0.5); }
    ...
    .doc-manage-modal { background: #1e1e2e; border: 1px solid #374151; border-radius: 12px; padding: 0; width: 380px; max-width: 90vw; box-shadow: 0 20px 60px rgba(0,0,0,0.5); overflow: hidden; }
    .doc-manage-header { padding: 16px 20px; border-bottom: 1px solid #374151; display: flex; align-items: center; justify-content: space-between; }
    .doc-manage-header h4 { margin: 0; font-size: 15px; color: #f3f4f6; font-weight: 600; }
    .doc-manage-close { background: none; border: none; color: #6b7280; font-size: 18px; cursor: pointer; padding: 0 4px; line-height: 1; }
    .doc-manage-close:hover { color: #f3f4f6; }
    .doc-manage-body { padding: 16px 20px; }
    .doc-manage-info { margin-bottom: 16px; padding: 12px; background: rgba(55,65,81,0.4); border-radius: 8px; }
    .doc-manage-info-title { font-size: 13px; color: #e5e7eb; font-weight: 600; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .doc-manage-info-meta { font-size: 11px; color: #6b7280; }
    .doc-manage-actions { display: flex; flex-direction: column; gap: 8px; }
    .doc-manage-btn { display: flex; align-items: center; gap: 10px; padding: 10px 14px; border-radius: 8px; border: 1px solid #374151; background: rgba(55,65,81,0.3); color: #d1d5db; font-size: 13px; cursor: pointer; transition: all 0.15s; }
    .doc-manage-btn:hover { background: rgba(55,65,81,0.6); }
    .doc-manage-btn.danger { border-color: rgba(239,68,68,0.3); }
    .doc-manage-btn.danger:hover { background: rgba(239,68,68,0.15); color: #fca5a5; border-color: rgba(239,68,68,0.5); }
    .doc-manage-btn-icon { font-size: 16px; flex-shrink: 0; }
    .doc-manage-btn-text { flex: 1; }
    .doc-manage-btn-desc { font-size: 11px; color: #6b7280; margin-top: 2px; }
    ...
    .add-doc-overlay { position: absolute; inset: 0; z-index: 100; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; }
    .add-doc-panel { width: 90%; max-width: 860px; max-height: 90vh; background: #1f2937; border: 1px solid #374151; border-radius: 12px; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,0.5); }
    .add-doc-header { display: flex; align-items: center; justify-content: space-between; padding: 16px 24px; border-bottom: 1px solid #374151; flex-shrink: 0; }
    .add-doc-header h3 { font-size: 16px; font-weight: 700; color: #f3f4f6; margin: 0; }
    .add-doc-close { background: none; border: none; color: #6b7280; font-size: 18px; cursor: pointer; padding: 4px 8px; border-radius: 4px; transition: all 0.15s; }
    .add-doc-close:hover { color: #f87171; background: rgba(248,113,113,0.1); }
    .add-doc-form { padding: 20px 24px; overflow-y: auto; flex: 1; display: flex; flex-direction: column; gap: 16px; }
    .add-doc-row { display: flex; gap: 16px; }
    .add-doc-row .add-doc-field { flex: 1; }
    .add-doc-field { display: flex; flex-direction: column; gap: 6px; }
    .add-doc-field label { font-size: 12px; font-weight: 600; color: #d1d5db; }
    .add-doc-field label .required { color: #f87171; }
    .add-doc-field label .optional { font-weight: 400; color: #6b7280; font-size: 11px; }
    .add-doc-field select, .add-doc-field input[type="text"] { background: #111827; border: 1px solid #374151; border-radius: 6px; padding: 8px 12px; color: #e5e7eb; font-size: 13px; outline: none; transition: border-color 0.2s; }
    .add-doc-field select:focus, .add-doc-field input[type="text"]:focus { border-color: #6366f1; }
    .add-doc-field select { cursor: pointer; }
    .add-doc-field select option { background: #1f2937; color: #e5e7eb; }
    .add-doc-content-field { flex: 1; min-height: 0; display: flex; flex-direction: column; }
    .add-doc-content-header { display: flex; align-items: center; justify-content: space-between; }
    .add-doc-shortcuts { font-size: 11px; color: #6b7280; }
    .add-doc-shortcuts kbd { background: #374151; border: 1px solid #4b5563; border-radius: 3px; padding: 1px 5px; font-size: 10px; color: #9ca3af; font-family: monospace; }
    .add-doc-textarea { flex: 1; min-height: 280px; max-height: 50vh; background: #111827; border: 1px solid #374151; border-radius: 8px; padding: 14px 16px; color: #e5e7eb; font-size: 13px; font-family: 'JetBrains Mono', 'Fira Code', 'Cascadia Code', 'Consolas', monospace; line-height: 1.6; resize: vertical; outline: none; transition: border-color 0.2s; tab-size: 2; }
    .add-doc-textarea:focus { border-color: #6366f1; }
    .add-doc-textarea::placeholder { color: #4b5563; }
    .add-doc-content-footer { display: flex; align-items: center; justify-content: space-between; margin-top: 4px; }
    .add-doc-char-count { font-size: 11px; color: #6b7280; }
    .add-doc-actions { display: flex; justify-content: flex-end; gap: 10px; padding-top: 4px; flex-shrink: 0; }
    .add-doc-btn { padding: 8px 18px; border-radius: 6px; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.15s; border: 1px solid transparent; }
    .add-doc-btn-cancel { background: none; border-color: #374151; color: #9ca3af; }
    .add-doc-btn-cancel:hover { border-color: #4b5563; color: #d1d5db; }
    .add-doc-btn-preview { background: rgba(59,130,246,0.12); border-color: rgba(59,130,246,0.3); color: #93c5fd; }
    .add-doc-btn-preview:hover { background: rgba(59,130,246,0.2); color: #bfdbfe; }
    .add-doc-btn-submit { background: rgba(16,185,129,0.15); border-color: rgba(16,185,129,0.3); color: #6ee7b7; }
    .add-doc-btn-submit:hover { background: rgba(16,185,129,0.25); color: #a7f3d0; }
    .add-doc-btn-batch { background: rgba(245,158,11,0.12); border-color: rgba(245,158,11,0.3); color: #fbbf24; }
    .add-doc-btn-batch:hover { background: rgba(245,158,11,0.2); color: #fde68a; }
    .add-doc-btn:disabled { opacity: 0.5; cursor: not-allowed; }

    /* Batch Import Progress Modal */
    .batch-import-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.55); z-index: 9100; display: none; align-items: center; justify-content: center; }
    .batch-import-overlay.active { display: flex; }
    .batch-import-modal { background: #1e1e2e; border: 1px solid #374151; border-radius: 12px; width: 520px; max-width: 92vw; max-height: 80vh; box-shadow: 0 20px 60px rgba(0,0,0,0.5); display: flex; flex-direction: column; overflow: hidden; }
    .batch-import-header { padding: 16px 20px; border-bottom: 1px solid #374151; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
    .batch-import-header h4 { margin: 0; font-size: 15px; color: #f3f4f6; font-weight: 600; }
    .batch-import-close { background: none; border: none; color: #6b7280; font-size: 18px; cursor: pointer; padding: 0 4px; line-height: 1; }
    .batch-import-close:hover { color: #f3f4f6; }
    .batch-import-body { padding: 16px 20px; overflow-y: auto; flex: 1; min-height: 0; }
    .batch-import-progress-wrap { margin-bottom: 14px; }
    .batch-import-progress-bar { height: 8px; background: #374151; border-radius: 4px; overflow: hidden; }
    .batch-import-progress-fill { height: 100%; background: linear-gradient(90deg, #6366f1, #818cf8); border-radius: 4px; transition: width 0.3s ease; }
    .batch-import-progress-text { font-size: 12px; color: #9ca3af; margin-top: 6px; text-align: center; }
    .batch-import-log { max-height: 280px; overflow-y: auto; font-size: 12px; font-family: 'Cascadia Code', 'Fira Code', monospace; background: rgba(0,0,0,0.3); border-radius: 8px; padding: 10px 12px; scrollbar-width: thin; scrollbar-color: #374151 transparent; }
    .batch-import-log::-webkit-scrollbar { width: 5px; }
    .batch-import-log::-webkit-scrollbar-thumb { background: #374151; border-radius: 3px; }
    .batch-import-log-item { padding: 3px 0; color: #9ca3af; line-height: 1.4; display: flex; align-items: flex-start; gap: 6px; }
    .batch-import-log-item.success { color: #6ee7b7; }
    .batch-import-log-item.error { color: #fca5a5; }
    .batch-import-log-item.skip { color: #fbbf24; }
    .batch-import-log-icon { flex-shrink: 0; width: 16px; text-align: center; }
    .batch-import-summary { margin-top: 14px; padding: 12px; background: rgba(99,102,241,0.1); border: 1px solid rgba(99,102,241,0.2); border-radius: 8px; font-size: 13px; color: #d1d5db; text-align: center; }
    ...
    /* Phase-69: 批量导入完整性检测报告 */
    .batch-verify-report { background: #111827; border: 1px solid #1f2937; border-radius: 8px; padding: 10px; font-size: 12px; }
    .batch-verify-summary { display: flex; gap: 16px; justify-content: center; flex-wrap: wrap; padding: 6px 0 8px; border-bottom: 1px solid #1f2937; margin-bottom: 8px; font-weight: 600; }
    .batch-verify-details { }
    .batch-verify-toggle { cursor: pointer; color: #60a5fa; font-size: 12px; padding: 4px 8px; border-radius: 4px; transition: background .15s; user-select: none; }
    .batch-verify-toggle:hover { background: rgba(59,130,246,0.1); }
    .batch-verify-item { padding: 8px 10px; margin: 4px 0; border-radius: 6px; border-left: 3px solid #374151; background: rgba(17,24,39,0.5); }
    .batch-verify-item.verify-error { border-left-color: #ef4444; background: rgba(239,68,68,0.05); }
    .batch-verify-item.verify-warning { border-left-color: #f59e0b; background: rgba(245,158,11,0.05); }
    .batch-verify-item-header { display: flex; align-items: center; gap: 6px; margin-bottom: 4px; }
    .batch-verify-icon { font-size: 14px; }
    .batch-verify-source { color: #d1d5db; font-weight: 500; font-size: 11px; word-break: break-all; flex: 1; }
    .batch-verify-type { font-size: 10px; padding: 1px 6px; background: rgba(99,102,241,0.15); color: #a5b4fc; border-radius: 3px; white-space: nowrap; }
    .batch-verify-issue { font-size: 11px; padding: 2px 0 2px 20px; line-height: 1.5; }
    .batch-verify-issue.error { color: #fca5a5; }
    .batch-verify-issue.warning { color: #fcd34d; }
    .batch-verify-metrics { display: flex; gap: 10px; flex-wrap: wrap; font-size: 10px; color: #6b7280; margin-top: 4px; padding-top: 4px; border-top: 1px solid rgba(31,41,55,0.5); }
    .batch-verify-metrics span { white-space: nowrap; }
`;
}
