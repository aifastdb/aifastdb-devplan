...
/**
 * Mermaid v11+ uses a stricter parser and no longer tolerates legacy labels
 * like node[foo [bar]] or node[text {meta}] unless the label is quoted.
 *
 * We only auto-quote plain square-bracket nodes. Special Mermaid shapes such
 * as [/db/] keep their original syntax so the rendered shape is preserved.
 */
function mdvPreprocessMermaid(source) {
  function mdvShouldQuoteSquareLabel(label) {
  ...
  function mdvQuoteSquareLabelsInLine(line) {
    var out = '';
    var i = 0;
    ...
    while (i < line.length) {
      ...
      for (; j < line.length; j++) {
        ...
        var prevCh = j > i + 1 ? line.charAt(j - 1) : '';

        if (quote) {
          if (ch === quote && prevCh !== '\\\\') quote = '';
          continue;
        }
      ...
      var label = line.slice(i + 1, j);
      if (!mdvShouldQuoteSquareLabel(label)) {
        out += '[' + label + ']';
        i = j + 1;
        continue;
      }
      ...
      while (start < end && /\\s/.test(label.charAt(start))) start += 1;
      while (end > start && /\\s/.test(label.charAt(end - 1))) end -= 1;

      var leading = label.slice(0, start);
      var core = label.slice(start, end).replace(/\\\\/g, '\\\\\\\\').replace(/"/g, '\\\\"');
      var trailing = label.slice(end);
  ...
  for (var i = 0; i < lines.length; i++) {
    lines[i] = mdvQuoteSquareLabelsInLine(lines[i]);
  }
  return lines.join('\\n');
}
