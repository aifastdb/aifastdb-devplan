...
const ALL_TOOLS = [
  ...
  {
    name: 'devplan_code_status',
    description: 'Get Code Intelligence status for a project repo. Returns native embedded code-graph availability, index status, and stats.\n获取项目代码智能状态。返回原生内嵌代码图的可用性、索引状态与统计信息。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: {
        ...
        repoPath: {
          type: 'string',
          description: 'Optional repository root path. If omitted, infer from the DevPlan base path.\n可选代码仓库根路径；省略时从 DevPlan base path 推断。',
        },
      },
      required: ['projectName'],
    },
  ...
  {
    name: 'devplan_code_graph',
    description: 'Export the embedded Code Intelligence graph for a repository. Returns code-graph nodes and edges.\n导出仓库的内嵌代码智能图，返回代码图节点与边。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
      required: ['projectName'],
    },
  },
  {
    name: 'devplan_code_clusters',
    description: 'List code communities/clusters discovered by the embedded Code Intelligence layer.\n列出内嵌代码智能层发现的代码 community/cluster。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_processes',
    description: 'List heuristic code processes discovered by the embedded Code Intelligence layer.\n列出内嵌代码智能层发现的 heuristic code processes。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_query',
    description: 'Search the embedded Code Intelligence graph by natural query text and return matching nodes, communities, and processes.\n按自然语言查询内嵌代码智能图，返回命中的节点、community 与 process。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_context',
    description: 'Get local graph neighborhood context for a symbol/file/community in the embedded Code Intelligence graph.\n获取内嵌代码智能图中某个 symbol/file/community 的局部上下文邻域。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
        symbol: { type: 'string', description: 'Target symbol or node query.\n目标符号或节点查询。' },
      },
      required: ['projectName', 'symbol'],
    },
  },
  {
    name: 'devplan_code_impact',
    description: 'Analyze upstream/downstream impact for a target symbol/file in the embedded Code Intelligence graph.\n分析内嵌代码智能图中某个目标 symbol/file 的上下游影响范围。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
        target: { type: 'string', description: 'Impact target symbol/file query.\n影响分析目标 symbol/file 查询。' },
        direction: {
          type: 'string',
          enum: ['upstream', 'downstream', 'both'],
          description: 'Traversal direction (default: both).\n遍历方向（默认 both）。',
        },
        limit: { type: 'number', description: 'Maximum affected nodes to return.\n最大返回影响节点数。' },
      },
      required: ['projectName', 'target'],
    },
  },
  {
    name: 'devplan_code_detect_changes',
    description: 'Compare the current source scan against the persisted embedded Code Intelligence index and return structured graph diffs.\n比较当前源码扫描结果与已持久化的内嵌代码智能索引，返回结构化图谱差异。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
        limit: { type: 'number', description: 'Maximum number of node/edge/community/process changes to return per category (default: 50).\n每类最多返回多少条变化（默认 50）。' },
      },
      required: ['projectName'],
    },
  },
  {
    name: 'devplan_code_rename',
    description: 'Preview or apply a graph-aware TS/JS symbol rename using the embedded Code Intelligence target resolution plus TypeScript rename locations.\n基于内嵌代码智能目标解析与 TypeScript rename locations，对 TS/JS symbol 执行预览或应用式重命名。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
        symbol: { type: 'string', description: 'Target symbol query when symbolId is not provided.\n未提供 symbolId 时使用的目标符号查询。' },
        symbolId: { type: 'string', description: 'Optional exact code symbol ID.\n可选的精确代码 symbol ID。' },
        newName: { type: 'string', description: 'New identifier name.\n新的标识符名称。' },
        apply: { type: 'boolean', description: 'If true, apply file edits. If false or omitted, preview only.\n为 true 时实际写入文件；否则仅预览。' },
      },
      required: ['projectName', 'newName'],
    },
  },
  {
    name: 'devplan_code_graph_query',
    description: 'Run a read-only structured subgraph query against the embedded Code Intelligence snapshot. This intentionally does not expose arbitrary cypher or graph mutation.\n对内嵌代码智能 snapshot 运行只读结构化子图查询；故意不暴露任意 cypher 或图变更能力。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
        query: { type: 'string', description: 'Optional text query for ranked node seeds.\n可选文本查询，用于命中节点种子。' },
        nodeTypes: { type: 'array', items: { type: 'string' }, description: 'Optional node type filter.\n可选节点类型过滤。' },
        edgeLabels: { type: 'array', items: { type: 'string' }, description: 'Optional edge label filter.\n可选边类型过滤。' },
        filePathPrefix: { type: 'string', description: 'Optional file path prefix filter.\n可选文件路径前缀过滤。' },
        communityId: { type: 'string', description: 'Optional community ID seed.\n可选 community ID 种子。' },
        processId: { type: 'string', description: 'Optional process ID seed.\n可选 process ID 种子。' },
        limit: { type: 'number', description: 'Maximum nodes to return (default: 25).\n最多返回节点数（默认 25）。' },
      },
      required: ['projectName'],
    },
  },
  {
    name: 'devplan_code_refactor_guardrails',
    description: 'Describe the current safe refactor boundaries for embedded Code Intelligence tools.\n描述当前内嵌代码智能工具的安全重构边界。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_rebuild_index',
    description: 'Rebuild and persist the embedded Code Intelligence index for a repository.\n重建并持久化仓库的内嵌代码智能索引。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_regression_check',
    description: 'Run native Code Intelligence regression comparison for a fixture repository and emit structured diff diagnostics.\n对 fixture 仓库运行原生代码智能回归比较，并输出结构化差异诊断。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        fixturePath: { type: 'string', description: 'Fixture repository root path to scan.\n要扫描的 fixture 仓库根路径。' },
        expectedPath: { type: 'string', description: 'Optional expected JSON path. Defaults to <fixturePath>/expected.json.\n可选 golden 期望 JSON 路径。默认使用 <fixturePath>/expected.json。' },
      },
      required: ['projectName', 'fixturePath'],
    },
  },
  {
    name: 'devplan_code_link_module',
    description: 'Create or refresh a mapping link between a DevPlan module and a Code Intelligence community.\n在 DevPlan 模块与代码 community 之间创建或刷新映射关系。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        moduleId: { type: 'string', description: 'DevPlan module ID.\nDevPlan 模块 ID。' },
        communityId: { type: 'string', description: 'Code Intelligence community ID.\n代码智能 community ID。' },
        note: { type: 'string', description: 'Optional bridge note.\n可选映射备注。' },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
      required: ['projectName', 'moduleId', 'communityId'],
    },
  },
  {
    name: 'devplan_code_resolve_module',
    description: 'Resolve code context mapped to a DevPlan module, including linked communities and processes.\n解析某个 DevPlan 模块映射到的代码上下文，包括 communities 与 processes。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_link_task',
    description: 'Create or refresh a mapping link between a DevPlan task and code symbols/processes.\n在 DevPlan 任务与代码 symbols/processes 之间创建或刷新映射关系。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        taskId: { type: 'string', description: 'DevPlan task ID.\nDevPlan 任务 ID。' },
        symbolIds: { type: 'array', items: { type: 'string' }, description: 'Code symbol/node IDs.\n代码 symbol/node ID 列表。' },
        processIds: { type: 'array', items: { type: 'string' }, description: 'Code process IDs.\n代码 process ID 列表。' },
        note: { type: 'string', description: 'Optional bridge note.\n可选映射备注。' },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
  ...
  {
    name: 'devplan_code_link_doc',
    description: 'Create or refresh a mapping link between a DevPlan document section and a code process.\n在 DevPlan 文档片段与代码 process 之间创建或刷新映射关系。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        section: { type: 'string', description: 'Document section type.\n文档片段类型。' },
        subSection: { type: 'string', description: 'Optional document subSection.\n可选文档子片段。' },
        processId: { type: 'string', description: 'Code process ID.\n代码 process ID。' },
        note: { type: 'string', description: 'Optional bridge note.\n可选映射备注。' },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
      required: ['projectName', 'section', 'processId'],
    },
  },
  {
    name: 'devplan_code_link_anchor',
    description: 'Create or refresh a mapping link between an anchor and a code symbol.\n在 Anchor 与代码 symbol 之间创建或刷新映射关系。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        anchorName: { type: 'string', description: 'Anchor name.\nAnchor 名称。' },
        symbolId: { type: 'string', description: 'Code symbol/node ID.\n代码 symbol/node ID。' },
        note: { type: 'string', description: 'Optional bridge note.\n可选映射备注。' },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
      required: ['projectName', 'anchorName', 'symbolId'],
    },
  },
  {
    name: 'devplan_code_recommend_module',
    description: 'Recommend community/process bridge candidates for a DevPlan module.\n为 DevPlan 模块推荐 community/process 桥接候选。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
        moduleId: { type: 'string', description: 'DevPlan module ID.\nDevPlan 模块 ID。' },
        limit: { type: 'number', description: 'Maximum recommendation count.\n最大推荐数。' },
        repoPath: { type: 'string', description: 'Optional repository root path.\n可选代码仓库根路径。' },
      },
      required: ['projectName', 'moduleId'],
    },
  ...
  {
    name: 'devplan_code_recommend_task',
    description: 'Recommend symbol/process bridge candidates for a DevPlan task.\n为 DevPlan 任务推荐 symbol/process 桥接候选。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_recommend_doc',
    description: 'Recommend symbol/process bridge candidates for a DevPlan document section.\n为 DevPlan 文档片段推荐 symbol/process 桥接候选。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
  ...
  {
    name: 'devplan_code_recommend_anchor',
    description: 'Recommend symbol/process bridge candidates for an anchor.\n为 Anchor 推荐 symbol/process 桥接候选。',
    inputSchema: {
      type: 'object' as const,
      properties: {
        projectName: { type: 'string', description: `Project name (default: "${DEFAULT_PROJECT_NAME}")\n项目名称（默认："${DEFAULT_PROJECT_NAME}"）` },
...
export interface ToolArgs {
  ...
  /** 功能模块 ID */
  moduleId?: string;
  /** Code Intelligence community ID */
  communityId?: string;
  /** Code Intelligence symbol ID */
  symbolId?: string;
  /** Code Intelligence symbol IDs */
  symbolIds?: string[];
  /** Code Intelligence process ID */
  processId?: string;
  /** Code Intelligence process IDs */
  processIds?: string[];
  /** Code Intelligence regression fixture repo path */
  fixturePath?: string;
  /** Code Intelligence regression expected json path */
  expectedPath?: string;
  /** Code bridge note */
  note?: string;
  /** 功能模块名称 */
  ...
  /** devplan_code_context: symbol query */
  symbol?: string;
  /** devplan_code_impact: target query */
  target?: string;
  /** devplan_code_impact: traversal direction */
  direction?: string;
  /** devplan_code_rename: 新标识符名称 */
  newName?: string;
  /** devplan_code_rename: 是否实际应用改动 */
  apply?: boolean;
  /** devplan_code_graph_query: 节点类型过滤 */
  nodeTypes?: string[];
  /** devplan_code_graph_query: 边类型过滤 */
  edgeLabels?: string[];
  /** devplan_code_graph_query: 文件路径前缀过滤 */
